...
void ErrorStateKalmanFilter::CorrectErrorEstimationPose(
  ...
  Eigen::Vector3d P_nn_obs = T_nb.block<3, 1>(0, 3) - pose_.block<3, 1>(0, 3);

  Eigen::Vector3d R_nn_obs = Sophus::SO3d::vee(pose_.block<3, 3>(0, 0).transpose() * T_nb.block<3, 3>(0, 0) - Eigen::Matrix3d::Identity());
  ...
  K = P_ * G.transpose() * (G * P_ * G.transpose() + CPose_ * R * CPose_.transpose()).inverse(); // fix this
