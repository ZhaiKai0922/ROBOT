...
private:
 ...
 static const int DIM_MEASUREMENT_POSE_NOISE = 6;
  ...
  typedef Eigen::Matrix<double, DIM_MEASUREMENT_POSI_VEL, DIM_STATE>
      MatrixGPosiVel;
  typedef Eigen::Matrix<double, DIM_MEASUREMENT_POSI_MAG, DIM_STATE>
      MatrixGPosiMag;
  typedef Eigen::Matrix<double, DIM_MEASUREMENT_POSI_VEL_MAG, DIM_STATE>
      MatrixGPosiVelMag;
  ...
  typedef Eigen::Matrix<double, DIM_MEASUREMENT_POSI_VEL,
      ...
      MatrixCPosiVel;
  typedef Eigen::Matrix<double, DIM_MEASUREMENT_POSI_MAG,
                        DIM_MEASUREMENT_POSI_MAG_NOISE>
      MatrixCPosiMag;
  typedef Eigen::Matrix<double, DIM_MEASUREMENT_POSI_VEL_MAG,
                        DIM_MEASUREMENT_POSI_VEL_MAG_NOISE>
      MatrixCPosiVelMag;
  ...
  typedef Eigen::Matrix<double, DIM_MEASUREMENT_POSI_VEL_NOISE,
                        DIM_MEASUREMENT_POSI_VEL_NOISE>
      MatrixRPosiVel;
  typedef Eigen::Matrix<double, DIM_MEASUREMENT_POSI_MAG_NOISE,
                        DIM_MEASUREMENT_POSI_MAG_NOISE>
      MatrixRPosiMag;
  typedef Eigen::Matrix<double, DIM_MEASUREMENT_POSI_VEL_MAG_NOISE,
                        DIM_MEASUREMENT_POSI_VEL_MAG_NOISE>
      MatrixRPosiVelMag;
  ...
  typedef Eigen::Matrix<double, DIM_MEASUREMENT_POSI_VEL_MAG, 1>
      VectorYPosiVelMag;
  ...
  typedef Eigen::Matrix<double, DIM_STATE, DIM_MEASUREMENT_POSI_VEL>
      MatrixKPosiVel;
  typedef Eigen::Matrix<double, DIM_STATE, DIM_MEASUREMENT_POSI_MAG>
      MatrixKPosiMag;
  typedef Eigen::Matrix<double, DIM_STATE, DIM_MEASUREMENT_POSI_VEL_MAG>
      MatrixKPosiVelMag;
  ...
  typedef Eigen::Matrix<double, DIM_STATE * DIM_MEASUREMENT_POSI_VEL, DIM_STATE>
      MatrixQPosiVel;
  typedef Eigen::Matrix<double, DIM_STATE * DIM_MEASUREMENT_POSI_MAG, DIM_STATE>
      MatrixQPosiMag;
  typedef Eigen::Matrix<double, DIM_STATE * DIM_MEASUREMENT_POSI_VEL_MAG,
                        DIM_STATE>
      MatrixQPosVelMag;
   ...
   * @param q_nb, orientation quaternioin
   * @return block matrix Fvq
   */
  Eigen::Matrix<double, 3, 4> GetFVelOri(const Eigen::Vector3d &f_b,
                          const Eigen::Quaterniond &q_nb);
  
  /**
   * @brief get block matrix for orientation quaternion update by orintation
   * quaternion
   * @param w_b, gyro measurement
   * @return block matrix Fqq
   */
  Eigen::Matrix<double, 4, 4>  GetFOriOri(const Eigen::Vector3d &w_b);

    /**
   * @brief  get block matrix for orientation quaternion update by epsilon,
   * angular velocity bias
   * @param  q_nb, orientation quaternion
   * @return block matrix Fqe
   */
  Eigen::Matrix<double, 4, 3>GetFOriEps(const Eigen::Quaternion &q_nb);
  ...
  GetUnbiasedAngularVel(const Eigen::Vector3d &angular_vel,
                        const Eigen::Matrix3d &C_nb);

  inline Eigen::Vector3d GetUnbiasedLinearAcc(const Eigen::Vector3d &linear_acc,
                                              const Eigen::Matrix3d &C_nb);      
   ...
   * @return f_b
   */
  inline Eigen::Vector3d RemoveGravity(const Eigen::Vector3d &f_b,
                                       const Eigen::Matrix3d &C_nb);
   ...
   * @return void
   */
  void CorrectStateEstimationPose(const Eigen::Matrix4d &T_nb);

  /**
   * @brief  correct state estimation using frontend pose & body velocity
   * measurement
   * @param  T_nb, input frontend pose estimation
   * @param  v_b, input odo
   * @return void
   */
  void CorrectStateEstimationPoseVel(const Eigen::Matrix4d &T_nb,
   ...
   */
  Eigen::Matrix<double, 3, 4> GetGMOri(const Eigen::Vector3d &m_n,
                                       const Eigen::Quaterniond &q_nb);

  /**
   * @brief  correct state estimation using GNSS position and odometer
   * measurement
   * @param  T_nb, input GNSS position
   * @param  v_b, input odo
   * @return void
   */
  void CorrectStateEstimationPosiVel(const Eigen::Matrix4d &T_nb,
                                     const Eigen::Vector3d &v_b);

  /**
   * @brief  correct state estimation using GNSS position and magneto
   * measurement
   * @param  T_nb, input GNSS position
   * @param  B_b, input magneto
   * @return void
   */
  void CorrectStateEstimationPosiMag(const Eigen::Matrix4d &T_nb,
                                     const Eigen::Vector3d &B_b);

  /**
   * @brief  correct state estimation using GNSS position, odometer and magneto
   * measurement
   * @param  T_nb, input GNSS position
   * @param  v_b, input odo
   * @param  B_b, input magneto
   * @return void
   */
  void CorrectStateEstimationPosiVelMag(const Eigen::Matrix4d &T_nb,
                                        const Eigen::Vector3d &v_b,
                                        const Eigen::Vector3d &B_b);
   ...
   * @return void
   */
  void CorrectStateEstimation(const MeasurementType &measurement_type,
                              const Measurement &measurement);
