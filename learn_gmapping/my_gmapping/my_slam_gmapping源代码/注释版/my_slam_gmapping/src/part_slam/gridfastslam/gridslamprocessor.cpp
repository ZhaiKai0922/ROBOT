...
namespace GMapping {
  ...
  bool GridSlamProcessor::processScan(const RangeReading & reading, int adaptParticles)
  {
    ...
    /*m_count表示这个函数被调用的次数 如果是第0次调用，上一次初始化上一次里程计位姿*/
    if (!m_count)
    {
      m_odoPose=relPose;
    }

	  /*对于每一个粒子，都从里程计运动模型中采样，更新每一个粒子对象存储的地图坐标系下的激光雷达位姿 */
    ...
    if (!m_count 
      ...
      if (m_count>0)
      {
        /*功能：为每一个粒子，在当前激光雷达位姿下，当前帧激光数据下，求解每个粒子的位姿最优值（策略：爬山，评价机制：得分机制（当前位姿下激光数据和当前地图的的匹配程度））
                求解每个粒子的权重
                爬山算法：每一层，从不同方向寻找一个得分最高的一个值，直到出现得分开始下降的地方（这个地方需要注意的是，细化第一次出现下降的量程，可以找到更优的）
                得分算法：根据当前位姿下的一帧激光扫描点和地图的匹配程度，对每一个激光点做计算，累积得分，返回得分
        */
        //通过扫描匹配寻找每一个粒子的最优位姿，并且计算粒子得分        
      ...
      else
      {
        ...
        {
          //输入当前地图，当前激光雷达位姿，当前激光雷达传感器数据
          //拓展地图大小，找到地图的有效区域，单位patch
          m_matcher.computeActiveArea(it->map, it->pose, plainReading);
          //为patch申请内存，更新每一个击中的栅格成员变量
